	...
	node(){
		sshagent (credentials: config.sshCreds) {
			stage ("${environment.toUpperCase()} Deployment") {
			   ...
			   echo "scp -o StrictHostKeyChecking=no -v '${WORKSPACE}/${deploymntRepoName}/envs/${environment}/${config.stackName}.yml' ${deployUser}@${deployServer}:${appStackYmlPath}"
			   echo "ssh -o StrictHostKeyChecking=no ${deployUser}@${deployServer} ${deployScript}"
			   
			   sh "scp -o StrictHostKeyChecking=no -v '${WORKSPACE}/${deploymntRepoName}/envs/${environment}/${config.stackName}.yml' ${deployUser}@${deployServer}:${appStackYmlPath}"
